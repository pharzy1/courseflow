...
await writeFile("data/catalog.snapshot.json",JSON.stringify({generatedAt:retrievedAt,totalAvailable:first.totalCount,records},null,2)+"\n");

if(process.env.DATABASE_URL){
  const db=getDb(),now=new Date(retrievedAt),sourceId="berkeleytime-public-graphql";
  await db.insert(sources).values({id:sourceId,name:"BerkeleyTime public GraphQL catalog",url:endpoint,official:false,retrievedAt:now,license:null,metadata:{term:`${semester} ${year}`,totalAvailable:first.totalCount,recordsImported:records.length}}).onConflictDoUpdate({target:sources.id,set:{retrievedAt:now,metadata:{term:`${semester} ${year}`,totalAvailable:first.totalCount,recordsImported:records.length}}});
  const uniqueCourses=[...new Map(records.map(record=>[record.courseId,record])).values()];
  for(let index=0;index<uniqueCourses.length;index+=200){const values=uniqueCourses.slice(index,index+200).map(record=>({id:record.courseId,subject:record.subject,number:record.number,title:record.title,description:record.description,department:record.department,unitsMin:String(record.unitsMin),unitsMax:String(record.unitsMax),level:record.level,prerequisites:record.prerequisites,requirementTags:record.requirements,crossListings:record.crossListings,sourceId,updatedAt:now}));await db.insert(courses).values(values).onConflictDoUpdate({target:courses.id,set:{title:sql`excluded.title`,description:sql`excluded.description`,unitsMin:sql`excluded.units_min`,unitsMax:sql`excluded.units_max`,level:sql`excluded.level`,requirementTags:sql`excluded.requirement_tags`,updatedAt:now}});}
  for(let index=0;index<records.length;index+=200){const batch=records.slice(index,index+200);await db.insert(sections).values(batch.map(record=>({id:record.id,courseId:record.courseId,term:record.term,sectionNumber:record.sectionNumber,component:record.component,instructors:[...new Set(record.meetings.flatMap(meeting=>meeting.instructors))],meetings:record.meetings.map(meeting=>({days:meeting.days,startTime:meeting.startTime,endTime:meeting.endTime,location:meeting.location})),enrolled:record.enrolled,capacity:record.capacity,waitlisted:record.waitlisted,waitlistCapacity:record.waitlistCapacity,sourceId,updatedAt:now}))).onConflictDoUpdate({target:sections.id,set:{component:sql`excluded.component`,instructors:sql`excluded.instructors`,meetings:sql`excluded.meetings`,enrolled:sql`excluded.enrolled`,capacity:sql`excluded.capacity`,waitlisted:sql`excluded.waitlisted`,waitlistCapacity:sql`excluded.waitlist_capacity`,updatedAt:now}});await db.insert(enrollmentSnapshots).values(batch.map(record=>({sectionId:record.id,observedAt:now,enrolled:record.enrolled,capacity:record.capacity,waitlisted:record.waitlisted,sourceId}))).onConflictDoNothing();}
  console.log(`Upserted ${uniqueCourses.length} courses and ${records.length} sections into Neon`);
}
console.log(`Wrote ${records.length} of ${first.totalCount} real catalog sections to data/catalog.snapshot.json`);
