...
test("official Coursedog parser fails closed when its export contract changes",()=>{
  assert.throws(()=>parseCoursedogMetadata('"Subject","Course Number"\n"DATA","C100"\n'),/missing required column: Course Title/);
});
...
test("hybrid adapter overlays official metadata without changing section facts",async()=>{
  ...
  const adapter=new HybridCatalogAdapter(sections,metadata),page=await adapter.fetchPage({year:2026,semester:"Fall",page:1,pageSize:50,timeoutMs:5000});
  ...
  assert.equal(page.results[0].courseMetadataOfficial,true);
  assert.deepEqual(await adapter.qualityEvidence(),{sampleSize:1,matchedCourses:1,coverage:1,catalogCourses:1,sectionCourses:1});
});
