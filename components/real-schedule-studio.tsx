...
export function ScheduleStudio(){
  const {selected,toggle}=useStudio(),[generated,setGenerated]=useState<ReturnType<typeof generateConflictFreeSchedules>>([]),[attempted,setAttempted]=useState(false),[open,setOpen]=useState(true),groups=useMemo(()=>{const result=new Map<string,CatalogRecord[]>();for(const item of selected)result.set(item.code,[...(result.get(item.code)??[]),item]);return result;},[selected]),comparisons=[...groups.entries()].filter(([,items])=>items.length>1);
  ...
  if(!open)return <button className="studio-launcher" onClick={()=>setOpen(true)} aria-label={`Open schedule studio with ${selected.length} selected sections`}><b>{selected.length}</b><span>Open schedule studio</span></button>;
  return <section className="schedule-studio drawer" aria-labelledby="studio-title"><header><div><span className="kicker">REAL SCHEDULE STUDIO</span><h2 id="studio-title">{selected.length} section{selected.length===1?"":"s"} · {groups.size} course{groups.size===1?"":"s"}</h2><p>Select multiple sections for a course to let CourseFlow choose the strongest conflict-free combination.</p></div><div className="studio-actions"><button className="minimize-studio" onClick={()=>setOpen(false)} aria-label="Minimize schedule studio">Minimize</button><button className="generate-real" onClick={generate}>Generate conflict-free schedules</button></div></header>
