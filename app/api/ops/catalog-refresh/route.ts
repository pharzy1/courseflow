...
export async function POST(request:Request){
  ...
  try{
    ...
    return Response.json({ok:true,generatedAt:result.generatedAt,sourceRows:result.sourceRows,distinctSections:result.distinctSections,courses:result.courses,pages:result.pages,durationMs:result.durationMs,alerts},{headers:{"cache-control":"no-store"}});
  }catch(error){
