...
export async function POST(request:Request){
  ...
  try{
    const result=await syncCatalog(),alerts=await evaluateEnrollmentAlerts(),academic=await syncAcademicProgram(),durationMs=Date.now()-started;await ensureAcademicSchema();await getDb().insert(refreshRuns).values({id:randomUUID(),kind:"production_ingestion",status:"success",startedAt,finishedAt:new Date(),durationMs,records:result.distinctSections,detail:`${academic.name} ${academic.degree} verified`});
    operationalLog("info","catalog_refresh_completed",{durationMs,sections:result.distinctSections,courses:result.courses,pages:result.pages,alertsCreated:alerts.created,emailsSent:alerts.emailed,academicProgram:academic.id});return Response.json({ok:true,generatedAt:result.generatedAt,sourceRows:result.sourceRows,distinctSections:result.distinctSections,courses:result.courses,pages:result.pages,durationMs:result.durationMs,alerts,academic:{id:academic.id,retrievedAt:academic.retrievedAt}},{headers:{"cache-control":"no-store"}});
  }catch(error){
