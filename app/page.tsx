...
export default function Home() {
  ...
  const compared=useMemo(()=>compareCourses(compare),[compare]);
  const focused=useMemo(()=>courses.find(course=>course.code===insightCourse)??courses[0],[insightCourse]);
  const focusedSignal=courseSignal(focused);
  const weeklyWorkload=selectedCourses.reduce((sum,course)=>sum+course.workload,0);
  ...
  return <main>
    ...
    <section className="workspace">
      <aside className="catalog" ref={catalogRef} id="discover">
        ...
        <div className="course-list">{filtered.length===0?<div className="empty-results"><b>No courses found</b><span>Try a course code like “CS 61B” or clear your filters.</span><button onClick={()=>{setQuery("");setQuickFilter("All courses");setDepartment("All departments");}}>Clear search and filters</button></div>:filtered.map(course=>{const picked=selected.includes(course.code);const signal=courseSignal(course);const comparing=compare.includes(course.code);return <article className={`course-card ${picked?"picked":""}`} key={course.code}><div className={`course-stripe ${course.color}`}/><div className="course-body"><div className="course-title"><div><span className="code">{course.code}</span><span className="units">{course.units} units</span><h3>{course.title}</h3></div><button onClick={()=>toggleCourse(course.code)} aria-pressed={picked} aria-label={`${picked?"Remove":"Add"} ${course.code}`}>{picked?"✓":"+"}</button></div><div className="tags">{course.tags.map(t=><span key={t}>{t}</span>)}</div><div className="course-meta"><span>◷ {course.days} · {course.time}</span><span>★ {course.rating} · {course.instructor}</span></div><div className="course-signals"><span><b>{course.grade}</b> median</span><span><b>{course.workload}h</b> weekly</span><span><b>{signal.fit}</b> fit</span></div><div className={`seats ${course.seats===0?"closed":course.seats<10?"low":""}`}><i/><span>{course.seats===0?"Waitlist only":`${course.seats} seats open`}</span><small>{course.seats}/{course.total}</small></div><div className="card-actions"><button aria-pressed={comparing} onClick={()=>toggleCompare(course.code)}>{comparing?"Comparing":"Compare"}</button><button onClick={()=>{setInsightCourse(course.code);navigate("Intelligence");}}>Full insight →</button></div></div></article>})}</div>
      </aside>
    ...
    <section className="intelligence" ref={intelligenceRef} id="intelligence" aria-labelledby="intelligence-title">
      <div className="intelligence-head"><div><span className="kicker">COURSE INTELLIGENCE</span><h2 id="intelligence-title">One decision, every signal.</h2><p>Grades, demand, prerequisites, workload, degree value, and schedule fit are connected here instead of scattered across separate tools.</p></div><label>Explore a course<select aria-label="Explore course intelligence" value={focused.code} onChange={event=>setInsightCourse(event.target.value)}>{courses.map(course=><option key={course.code}>{course.code}</option>)}</select></label></div>
      <div className="intelligence-grid">
        <article className="insight-primary"><div className="insight-title"><div><span className={`signal-dot ${focused.color}`}/><div><span>{focused.code}</span><h3>{focused.title}</h3></div></div><strong>{focusedSignal.fit}<small>/100 fit</small></strong></div><p>{focused.description}</p><div className="signal-grid"><div><span>Historical median</span><b>{focused.grade}</b><small>Illustrative distribution</small></div><div><span>Weekly workload</span><b>{focused.workload}h</b><small>Estimated outside class</small></div><div><span>Enrollment pressure</span><b>{focused.demand}%</b><small>{focusedSignal.risk}</small></div><div><span>Instructor signal</span><b>{focused.rating}</b><small>out of 5</small></div></div><div className="trend"><div><b>Enrollment momentum</b><span>Four checkpoints</span></div><div className="trend-bars" aria-label={`${focused.code} illustrative enrollment trend ${focused.trend.join(", ")} percent`}>{focused.trend.map((value,index)=><i key={index} style={{height:`${value}%`}}><span>{value}%</span></i>)}</div></div></article>
        <article className="path-card"><span className="kicker">PREREQUISITE PATH</span><h3>Know what unlocks what.</h3>{focused.prerequisites.length?<div className="prereq-flow">{focused.prerequisites.map(prerequisite=><span key={prerequisite}>{prerequisite}</span>)}<i>→</i><b>{focused.code}</b></div>:<p>No listed prerequisites in this sample.</p>}<span className="kicker">DEGREE VALUE</span><ul>{focused.requirements.map(requirement=><li key={requirement}>✓ {requirement}</li>)}</ul></article>
        <article className="load-card"><span className="kicker">SEMESTER HEALTH</span><h3>{weeklyWorkload} hours outside class</h3><div className="load-meter"><i style={{width:`${Math.min(100,weeklyWorkload/60*100)}%`}}/></div><p>{weeklyWorkload>50?"Heavy load—consider balancing one technical course.":weeklyWorkload>35?"Ambitious but balanced for a full-time semester.":"Plenty of capacity remains in this plan."}</p><div><b>{requirements.length}</b><span>requirement areas advanced</span></div></article>
      </div>
      <div className="comparison"><div><span className="kicker">DECISION DESK</span><h3>Compare up to three courses</h3><p>Use the Compare controls in the catalog. Adding a fourth automatically rotates out the oldest choice.</p></div><div className="compare-table" role="region" aria-label="Course comparison" tabIndex={0}><table><thead><tr><th>Course</th>{compared.map(course=><th key={course.code}>{course.code}<button aria-label={`Remove ${course.code} from comparison`} onClick={()=>toggleCompare(course.code)}>×</button></th>)}</tr></thead><tbody><tr><th>Decision fit</th>{compared.map(course=><td key={course.code}><b>{course.fit}</b>/100</td>)}</tr><tr><th>Median grade</th>{compared.map(course=><td key={course.code}>{course.grade}</td>)}</tr><tr><th>Weekly load</th>{compared.map(course=><td key={course.code}>{course.workload}h</td>)}</tr><tr><th>Seat outlook</th>{compared.map(course=><td key={course.code}>{course.risk}</td>)}</tr><tr><th>Requirements</th>{compared.map(course=><td key={course.code}>{course.requirements.length}</td>)}</tr></tbody></table></div></div>
      <p className="data-note">Prototype intelligence uses transparent illustrative data—not official CalCentral, BerkeleyTime, or enrollment predictions.</p>
    </section>
