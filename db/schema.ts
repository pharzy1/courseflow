...
export const enrollmentAlerts=pgTable("enrollment_alerts",{
  id:text("id").primaryKey(),watchId:text("watch_id").notNull().references(()=>enrollmentWatches.id),userId:text("user_id").notNull().references(()=>userProfiles.id),sectionId:text("section_id").notNull().references(()=>sections.id),kind:text("kind").notNull(),message:text("message").notNull(),dedupeKey:text("dedupe_key").notNull().unique(),readAt:timestamp("read_at",{withTimezone:true}),emailStatus:text("email_status").notNull().default("not_requested"),emailAttempts:integer("email_attempts").notNull().default(0),providerMessageId:text("provider_message_id"),lastEmailAttemptAt:timestamp("last_email_attempt_at",{withTimezone:true}),createdAt:timestamp("created_at",{withTimezone:true}).notNull(),
},table=>[index("enrollment_alerts_user_time_idx").on(table.userId,table.createdAt)]);
export const notificationPreferences=pgTable("notification_preferences",{userId:text("user_id").primaryKey().references(()=>userProfiles.id),emailEnabled:boolean("email_enabled").notNull().default(false),frequency:text("frequency").notNull().default("immediate"),quietStart:integer("quiet_start").notNull().default(22),quietEnd:integer("quiet_end").notNull().default(7),timezone:text("timezone").notNull().default("America/Los_Angeles"),unsubscribedAt:timestamp("unsubscribed_at",{withTimezone:true}),updatedAt:timestamp("updated_at",{withTimezone:true}).notNull()});
export const degreeRoadmaps=pgTable("degree_roadmaps",{
  id:text("id").primaryKey(),userId:text("user_id").notNull().references(()=>userProfiles.id).unique(),payload:jsonb("payload").$type<Record<string,unknown>>().notNull().default({}),createdAt:timestamp("created_at",{withTimezone:true}).notNull(),updatedAt:timestamp("updated_at",{withTimezone:true}).notNull(),
});
export const academicPrograms=pgTable("academic_programs",{id:text("id").primaryKey(),name:text("name").notNull(),degree:text("degree").notNull(),sourceUrl:text("source_url").notNull(),official:boolean("official").notNull(),requirements:jsonb("requirements").$type<Array<{group:string;rule:string;courses:string[]}>>().notNull(),retrievedAt:timestamp("retrieved_at",{withTimezone:true}).notNull()});
export const refreshRuns=pgTable("refresh_runs",{id:text("id").primaryKey(),kind:text("kind").notNull(),status:text("status").notNull(),startedAt:timestamp("started_at",{withTimezone:true}).notNull(),finishedAt:timestamp("finished_at",{withTimezone:true}).notNull(),durationMs:integer("duration_ms").notNull(),records:integer("records").notNull().default(0),detail:text("detail")},table=>[index("refresh_runs_kind_time_idx").on(table.kind,table.startedAt)]);
