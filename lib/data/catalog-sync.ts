...
export async function syncCatalog(options:{writeDatabase?:boolean}={}):Promise<CatalogSyncResult>{
  ...
  async function fetchPage(page:number):Promise<SourcePage>{
    ...
    for(let attempt=1;attempt<=3;attempt++)try{
      const response=await fetch(endpoint,{method:"POST",signal:AbortSignal.timeout(timeout),headers:{accept:"application/json","content-type":"application/json",origin:"https://berkeleytime.com",referer:"https://berkeleytime.com/","user-agent":"Mozilla/5.0 (compatible; CourseFlow/1.0; +https://github.com/pharzy1/courseflow)"},body:JSON.stringify({query:catalogQuery,variables:{year,semester,page,pageSize}})});
