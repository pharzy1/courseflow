...
export type SourceCatalogRecord={termId?:string;sessionId:string;subject:string;courseNumber:string;number:string;courseId?:string;title?:string|null;courseTitle?:string|null;courseDescription?:string|null;courseMetadataOfficial?:boolean;unitsMin:number;unitsMax:number;level?:string|null;breadthRequirements?:string[];universityRequirements?:string[];crossListings?:string[];enrolledCount?:number;maxEnroll?:number;waitlistedCount?:number;maxWaitlist?:number;allTimeAverageGrade?:number|null;primaryComponent?:string|null;meetings?:SourceMeeting[]};
...
export type SourceQualityEvidence={sampleSize:number;matchedCourses:number;coverage:number;catalogCourses:number;sectionCourses:number};
