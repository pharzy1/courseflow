...
export class NeonCourseRepository implements CourseDataRepository{
  async searchCatalog(query:CatalogQuery):Promise<CatalogPage>{
    ...
    const base=db.select({course:courses,section:sections,source:sources,grade:gradeDistributions}).from(sections).innerJoin(courses,eq(sections.courseId,courses.id)).innerJoin(sources,eq(courses.sourceId,sources.id)).leftJoin(gradeDistributions,gradeJoin).where(and(...conditions));
    ...
    return {records:rows.map(({course,section,source,grade})=>({id:section.id,courseId:course.id,code:`${course.subject} ${course.number}`,subject:course.subject,number:course.number,title:course.title,description:course.description,department:course.department,unitsMin:Number(course.unitsMin),unitsMax:Number(course.unitsMax),level:course.level,requirements:course.requirementTags,prerequisites:course.prerequisites,crossListings:course.crossListings,term:section.term,sectionNumber:section.sectionNumber,component:section.component,meetings:section.meetings.map(meeting=>({...meeting,instructors:section.instructors})),enrolled:section.enrolled,capacity:section.capacity,waitlisted:section.waitlisted,waitlistCapacity:section.waitlistCapacity,averageGrade:grade?.mean===null||grade?.mean===undefined?null:Number(grade.mean),medianGrade:grade?.median??null,gradeSampleSize:grade?.sampleSize??0,provenance:{sourceId:source.id,sourceName:source.name,sourceUrl:source.url,official:source.official,retrievedAt:source.retrievedAt.toISOString(),license:source.license}})),total:Number(count),mode:"neon",generatedAt,facets:{departments:departmentRows.map(row=>row.department)}};
  }
}
