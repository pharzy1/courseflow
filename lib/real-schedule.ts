...
function minutes(value:string|null){if(!value)return null;const match=value.match(/^(\d{1,2}):(\d{2})/);if(!match)return null;const result=Number(match[1])*60+Number(match[2]);return result===0?null:result;}

export function meetingsConflict(left:Meeting,right:Meeting){
  if(!left.days.some(day=>right.days.includes(day)))return false;
  const leftStart=minutes(left.startTime),leftEnd=minutes(left.endTime),rightStart=minutes(right.startTime),rightEnd=minutes(right.endTime);
